...
export function mapJsonSchemaToFields(schema: JsonSchema): FieldMetadata[] {
  ...
  Object.entries(schema.properties).forEach(([fieldName, property]) => {
    try {
      const fieldMetadata = mapPropertyToFieldMetadata(fieldName, property, schema.required || []);
      if (fieldMetadata) {
        fields.push(fieldMetadata);
      }
    } catch (error) {
      console.error(`[JsonSchemaMapper] Erro ao mapear campo '${fieldName}':`, error);
    }
  });
...
export function mapPropertyToFieldMetadata(
  ...
  if (property.$ref && (!property['x-ui'] || property['x-ui'] === null)) {
    console.debug(`[JsonSchemaMapper] Pulando campo '${fieldName}' - referência complexa sem x-ui`);
    return null;
  }

  // Pular campos do tipo object sem x-ui ou com x-ui null (estruturas aninhadas)
  if (property.type === 'object' && (!property['x-ui'] || property['x-ui'] === null)) {
    console.debug(`[JsonSchemaMapper] Pulando campo '${fieldName}' - objeto aninhado sem x-ui`);
    return null;
  }

  // Pular campos do tipo array sem x-ui ou com x-ui null
  if (property.type === 'array' && (!property['x-ui'] || property['x-ui'] === null)) {
    console.debug(`[JsonSchemaMapper] Pulando campo '${fieldName}' - array sem x-ui`);
    return null;
  }
  ...
  const controlType = uiConfig.controlType || inferControlTypeFromSchema(property);
  
  if (!controlType) {
    console.warn(`[JsonSchemaMapper] Não foi possível determinar controlType para '${fieldName}'`);
    return null;
  }
 ...
 */
function inferControlTypeFromSchema(property: JsonSchemaProperty): string | null {
  ...
  switch (property.type) {
    ...
    default:
      console.debug(`[JsonSchemaMapper] Tipo '${property.type}' não reconhecido, usando input como fallback`);
      return 'input';
  }
}
 ...
 */
function extractTypeSpecificProperties(property: JsonSchemaProperty, uiConfig: any): any {
  const specificProps: any = {};

  // Propriedades de validação
  if (property.minLength !== undefined) specificProps.minLength = property.minLength;
  if (property.maxLength !== undefined) specificProps.maxLength = property.maxLength;
 ...
 */
export function normalizeFormMetadata(input: FieldMetadata[] | JsonSchema): FieldMetadata[] {
  ...
  console.error('[JsonSchemaMapper] Input não é nem FieldMetadata[] nem JsonSchema válido:', input);
  return [];
}