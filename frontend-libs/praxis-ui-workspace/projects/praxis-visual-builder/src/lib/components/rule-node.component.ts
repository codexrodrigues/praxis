...
import { FieldSchema, FIELD_TYPE_OPERATORS, OPERATOR_LABELS } from '../models/field-schema.model';
...
@Component({
  ...
  template: `
    <div class="rule-node-container"
         [class.selected]="isSelected"
         [class.has-errors]="hasValidationErrors"
         [class]="'level-' + level">
      
      <!-- Main Rule Node -->
      <mat-card class="rule-node" 
                [class.selected]="isSelected"
                (click)="selectNode()">
        ...
        <div class="node-header">
          ...
          <div class="node-actions">
            ...
            <mat-icon *ngIf="hasValidationErrors" 
                      class="error-badge"
        ...
        <div class="node-content">
          ...
          <div *ngIf="isBooleanGroup()" class="boolean-group-content">
            <div class="group-operator">
              <mat-form-field appearance="outline" class="operator-select">
                <mat-select [value]="getBooleanOperator()" 
                           (selectionChange)="onBooleanOperatorChanged($event.value)">
          ...
          <div *ngIf="isConditionalValidator()" class="conditional-validator-content">
          ...
          <div *ngIf="isCollectionValidation()" class="collection-validation-content">
          ...
          <div *ngIf="node?.metadata" class="metadata-preview">
            <mat-chip-set class="metadata-chips">
              ...
              <mat-chip *ngIf="node.metadata.tag" 
        ...
        <div *ngIf="hasValidationErrors" class="validation-errors">
          <div *ngFor="let error of validationErrors" 
            ...
            <mat-icon class="error-icon">{{ getErrorIcon(error.severity) }}</mat-icon>
      ...
      <div *ngIf="hasChildren()" class="children-container">
        ...
        <div class="children-list"
          ...
          <div *ngFor="let childId of node?.children; trackBy: trackByChildId"
               class="child-wrapper"
               cdkDrag>
  ...
  styles: [`
    ...
    .type-icon.field-condition { color: var(--mdc-theme-primary); }
    .type-icon.boolean-group { color: var(--mdc-theme-secondary); }
    .type-icon.conditional-validator { color: var(--mdc-theme-tertiary); }
    .type-icon.collection-validation { color: var(--mdc-theme-error); }
    ...
    .children-list.cdk-drop-list-dragging .child-wrapper:not(.cdk-drag-placeholder) {
      transition: transform 250ms cubic-bezier(0, 0, 0.2, 1);
    }
...
export class RuleNodeComponent {
  ...
  @Output() childMoved = new EventEmitter<any>();
  ...
  getCollectionValidationConfig(): any {
    if (!this.isCollectionValidation()) return null;
    return this.node?.config || null;
  }
  ...
  getChildOptions(): { type: RuleNodeType; icon: string; label: string }[] {
    return [
      { type: RuleNodeType.FIELD_CONDITION, icon: 'compare_arrows', label: 'Field Condition' },
  ...
  getChildValidationErrors(childId: string): any[] {
    // This would typically come from a service or parent component
    return [];
  }
  ...
  onBooleanOperatorChanged(operator: string): void {
    if (!this.node) return;
    
    const config: BooleanGroupConfig = {
      ...this.getBooleanGroupConfig(),
      type: 'booleanGroup',
      operator: operator as 'and' | 'or' | 'not' | 'xor' | 'implies'
    };
  ...
  onCollectionValidationChanged(config: any): void {
    if (!this.node) return;
  ...
  // Helper methods
  private formatNodeType(type: RuleNodeType): string {
    return type.split(/(?=[A-Z])/).join(' ').toLowerCase()
