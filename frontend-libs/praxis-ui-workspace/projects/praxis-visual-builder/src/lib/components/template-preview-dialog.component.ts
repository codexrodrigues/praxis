      ...
      <div mat-dialog-title class="dialog-title">
        ...
        <div class="title-actions">
          <mat-chip class="complexity-chip" 
                   [class]="'complexity-' + (data.template.metadata?.complexity || 'unknown')">
      ...
      <div mat-dialog-content class="dialog-content">
        ...
        <div class="template-metadata">
          ...
          <div class="metadata-grid">
            ...
            <div class="metadata-item" *ngIf="data.template.metadata?.usageCount">
              <mat-icon>trending_up</mat-icon>
              <span class="label">Usage Count:</span>
              <span class="value">{{ data.template.metadata.usageCount }}</span>
            </div>
            <div class="metadata-item" *ngIf="data.template.metadata?.createdAt">
              <mat-icon>schedule</mat-icon>
              <span class="label">Created:</span>
              <span class="value">{{ formatDate(data.template.metadata.createdAt) }}</span>
            </div>
            <div class="metadata-item" *ngIf="data.template.metadata?.updatedAt">
              <mat-icon>update</mat-icon>
              <span class="label">Updated:</span>
              <span class="value">{{ formatDate(data.template.metadata.updatedAt) }}</span>
            </div>
            <div class="metadata-item" *ngIf="data.template.metadata?.author?.name">
              <mat-icon>person</mat-icon>
              <span class="label">Author:</span>
              <span class="value">{{ data.template.metadata.author.name }}</span>
            </div>
        ...
        <div class="rule-structure">
          ...
          <div class="structure-container">
            <mat-expansion-panel *ngFor="let rootNodeId of data.template.rootNodes; let i = index"
                                [expanded]="i === 0"
                                class="root-node-panel">
              <mat-expansion-panel-header>
                <mat-panel-title>
                  <mat-icon>{{ getNodeIcon(getRootNode(rootNodeId)) }}</mat-icon>
                  <span>{{ getRootNode(rootNodeId)?.label || getRootNode(rootNodeId)?.type || 'Unknown Node' }}</span>
                </mat-panel-title>
                ...
                <div class="node-tree">
                  <div class="tree-node" 
                    ...
                    <div class="node-content">
                      <mat-icon class="node-icon">{{ getNodeIcon(node.node) }}</mat-icon>
                      <span class="node-label">{{ node.node.label || node.node.type }}</span>
                      <mat-chip class="node-type-chip" size="small">{{ node.node.type }}</mat-chip>
                    </div>
                    <div class="node-config" *ngIf="node.node.config">
                      <pre class="config-preview">{{ formatConfig(node.node.config) }}</pre>
                    </div>
                  </div>
                </div>
              </div>
