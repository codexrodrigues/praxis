        ...
        <div class="tester-grid">
              ...
              <div class="test-options">
                ...
                <div class="test-option">
                  ...
                  <div class="no-rule-info" *ngIf="!hasCurrentRule">
                    <span class="no-rule-message">No rule loaded in builder</span>
                  </div>
                </div>
                ...
                <div class="test-option">
                  ...
                  <div class="test-suite-info">
                    <span class="test-count">{{ defaultTestCases.length }} test cases</span>
                    <button mat-button 
                            size="small"
                  ...
                  <div *ngIf="showTestCases" class="test-cases-list">
                    <div *ngFor="let testCase of defaultTestCases" class="test-case-item">
                      <div class="test-case-header">
                        <span class="test-case-name">{{ testCase.name }}</span>
                        <mat-chip [color]="getTestCaseColor(testCase)" size="small">
          ...
          <mat-card class="results-panel">
            <mat-card-header>
              <mat-card-title>
                Test Results
                <mat-icon *ngIf="isRunning" class="running-icon">hourglass_empty</mat-icon>
              </mat-card-title>
            ...
            <mat-card-content>
              ...
              <!-- No Results Message -->
              <div *ngIf="!lastTestResult && !testSuiteResults && !isRunning" 
                   class="no-results">
              ...
              <!-- Single Test Result -->
              <div *ngIf="lastTestResult && !testSuiteResults" class="single-test-result">
                <div class="result-header">
                  <div class="result-status" [class]="getStatusClass(lastTestResult)">
                    <mat-icon>{{ getStatusIcon(lastTestResult) }}</mat-icon>
                    <span>{{ lastTestResult.success ? 'PASSED' : 'FAILED' }}</span>
                  </div>
                ...
                <mat-expansion-panel class="stages-panel">
                  ...
                  <div class="stages-list">
                    <div *ngFor="let stage of getStages(lastTestResult)" 
                         class="stage-item"
                         [class]="stage.success ? 'stage-success' : 'stage-error'">
                      <div class="stage-header">
                        <mat-icon>{{ stage.success ? 'check_circle' : 'error' }}</mat-icon>
                        <span class="stage-name">{{ stage.name }}</span>
                        <span class="stage-timing">{{ stage.timing?.toFixed(2) }}ms</span>
                      </div>
                ...
                <mat-expansion-panel class="integrity-panel">
                  ...
                  <div class="integrity-checks">
                    <div *ngFor="let check of getIntegrityChecks(lastTestResult)" 
                         class="integrity-check"
                         [class]="check.passed ? 'check-passed' : 'check-failed'">
                      <mat-icon>{{ check.passed ? 'check' : 'close' }}</mat-icon>
                ...
                <!-- Errors and Warnings -->
                <div *ngIf="lastTestResult.errors.length > 0 || lastTestResult.warnings.length > 0" 
                     class="issues-section">
                  
                  <!-- Errors -->
                  <div *ngIf="lastTestResult.errors.length > 0" class="errors-section">
                    <h4 class="section-title error-title">
                      <mat-icon>error</mat-icon>
                      Errors ({{ lastTestResult.errors.length }})
                    </h4>
                    <div *ngFor="let error of lastTestResult.errors" class="issue-item error-item">
                  ...
                  <!-- Warnings -->
                  <div *ngIf="lastTestResult.warnings.length > 0" class="warnings-section">
                    <h4 class="section-title warning-title">
                      <mat-icon>warning</mat-icon>
                      Warnings ({{ lastTestResult.warnings.length }})
                    </h4>
                    <div *ngFor="let warning of lastTestResult.warnings" class="issue-item warning-item">
                      <div class="issue-header">
                        <span class="issue-code">{{ warning.code }}</span>
                        <span class="issue-severity">{{ warning.severity }}</span>
                      </div>
              ...
              <div *ngIf="testSuiteResults" class="test-suite-results">
                <div class="suite-summary">
                  <div class="summary-stats">
                    <div class="stat-item">
                      ...
                      <span class="stat-value">{{ testSuiteResults.totalTests }}</span>
                    </div>
                    <div class="stat-item passed">
                      <span class="stat-label">Passed</span>
                      <span class="stat-value">{{ testSuiteResults.passed }}</span>
                    </div>
                    <div class="stat-item failed">
                      <span class="stat-label">Failed</span>
                      <span class="stat-value">{{ testSuiteResults.failed }}</span>
                    </div>
                  </div>
                ...
                <mat-expansion-panel *ngFor="let result of testSuiteResults.results; let i = index" 
                                     class="test-result-panel">
                  <mat-expansion-panel-header>
                    <mat-panel-title>
                      <mat-icon [class]="result.result.success ? 'success-icon' : 'error-icon'">
                    ...
                    <mat-panel-description>
                      ...
                      <span class="test-timing">{{ result.result.performance.totalTime.toFixed(2) }}ms</span>
                    </mat-panel-description>
                  </mat-expansion-panel-header>
                    ...
                    <div class="test-case-info">
                      ...
                      <div class="test-case-details">
                        <span class="detail-item">
                          <strong>Type:</strong> {{ result.testCase.visualRule.type }}
                        </span>
                        <span class="detail-item">
                    ...
                    <div class="quick-summary">
                      ...
                      <div class="summary-items">
                        <span class="summary-item" 
                              [class]="result.result.success ? 'success' : 'failure'">
                          <mat-icon>{{ result.result.success ? 'check' : 'close' }}</mat-icon>
...
export class RoundTripTesterComponent implements OnInit {
  ...
  runCurrentRuleTest(): void {
    if (!this.hasCurrentRule) {
      this.snackBar.open('No rule loaded in builder', 'Close', { duration: 3000 });
      return;
    }
      ...
      try {
        ...
        if (state.rootNodes.length > 0) {
          ...
          if (rootNode) {
            ...
            const completeRule = this.buildCompleteRuleTree(rootNode, state.nodes);
            this.lastTestResult = this.roundTripValidator.validateRoundTrip(completeRule);
        ...
        this.snackBar.open(`Test failed: ${error}`, 'Close', { duration: 4000 });
      } finally {
  ...
  runTestSuite(): void {
    ...
    setTimeout(() => {
      try {
        this.testSuiteResults = this.roundTripValidator.runTestSuite(this.defaultTestCases);
        
        const message = `Test suite completed: ${this.testSuiteResults.passed}/${this.testSuiteResults.totalTests} passed`;
        this.snackBar.open(message, 'Close', { duration: 3000 });
      } catch (error) {
        this.snackBar.open(`Test suite failed: ${error}`, 'Close', { duration: 4000 });
      } finally {
        this.isRunning = false;
        this.cdr.detectChanges();
      }
  ...
  private buildCompleteRuleTree(node: RuleNode, allNodes: Record<string, RuleNode>): RuleNode {
    return {
      ...node,
  ...
  getSuccessRate(): number {
    ...
    return Math.round((this.testSuiteResults.passed / this.testSuiteResults.totalTests) * 100);
  }
